...
- `APP.md` covers the signed-in screens. They render only from `lib/app/data.ts`; never fetch inside app components, so the live app can swap the data module. App styles stay in `app/(app)/app.css` under `.app`.
